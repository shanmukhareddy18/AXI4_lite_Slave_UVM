...
task reference();
`uvm_info("SCB",$sformatf("INP_TX-> reset=%b AWADDR=%b ,AWPROT=%b ,AWVALID=%b ,WDATA=%b ,WVALID=%b ,WSTRB=%b ,BREADY=%b ,ARADDR=%b ,ARPROT=%b ,ARVALID=%b ,RREADY=%b",
...
`uvm_info("OUT_MON", $sformatf("OUT_TX -> AWREADY:%b | WREADY:%0b | BRESP:%0b | BVALID:%0b | ARREADY:%b | RDATA:%0b | RRESP:%0b | RVALID:%0b  ",
...
if(!inp_tx.reset)begin
 r_data=0;
  BRESP=0;BVALID=0;r_data=0; RRESP=0;RVALID=0;end
else
 begin
   if(RVALID)begin
        RVALID=0;RRESP=0; end
   if(BVALID)begin
   ...
   if(waddr_done && wdata_done) begin
     ...
     if(addr > 32'h3C)
       BRESP=2'b11;
        ...
        BRESP=2'b00;
        mem[addr]={ {8{wstrb[3]}},{8{wstrb[2]}},{8{wstrb[1]}},{8{wstrb[0]}} }&w_data; 
       end
    end
   ...
   if(inp_tx.ARVALID && out_tx.ARREADY)
    begin
     ...
     if(inp_tx.ARADDR > 32'h3C)
       ...
       RRESP=2'b00;
       r_data=mem[inp_tx.ARADDR];
       end
    end
 
 end
 `uvm_info("SCB", $sformatf(
      "EXP -> waddr_done=%b, wdata_done=%b| BRESP:%0b | BVALID:%0b |  RDATA:%0b | RRESP:%0b | RVALID:%0b  ",
       waddr_done,wdata_done,BRESP, BVALID, exp.RDATA, exp.RRESP, exp.RVALID), UVM_NONE)
  if(exp.RDATA==out_tx.RDATA && exp.RRESP==out_tx.RRESP && exp.RVALID==out_tx.RVALID)
   begin
    success++; $display("SUCCESS");end
 else
  begin
